...
(async () => {
  ...
  // DESIGN-002: the site list lives in the Filters menu and its quick button. The rows,
  // the keep-one rule and "which sites are on" did not change with the move.
  {
    const head = element();
    const hint = element();
    const host = element();
    ...
    const writes = [];
    const railConfig = { onlineSources: OnlineSources.normalize({ internet: true }, providers) };
    const sectionCtx = {
      ...
      renderOnlineQuickFilters() { sectionCtx.quickRenders = (sectionCtx.quickRenders || 0) + 1; },
      Promise,
      window: { api: { setConfig: async (patch) => { writes.push(patch); return {}; } } },
      $: (id) => ({
        '#onlineQuickSources': head, '#onlineSourcesHint': hint, '#onlineSourceOptions': host,
      })[id] || null,
    };
    ...
    assert.strictEqual(head.title, 'One, Two', 'the quick button names the sites that are on');
    assert.strictEqual(head.disabled, false);
    assert.strictEqual(hint.hidden, true, 'the keep-one rule is quiet while it holds nothing back');
    assert.strictEqual(writes.length, 0, 'showing the sites writes nothing');
    assert.ok(sectionCtx.quickRenders >= 1, 'the quick row learns about the change');
  ...
  console.log('PASS: compact rail, MD5 across pages, source save/race/rollback, preserved filters, the site list in Filters and uncut titles');
